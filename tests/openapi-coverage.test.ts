...
describe("indexOpenApiOperations", () => {
  it.fails(
    "returns >=136 operations with METHOD /path keys (OAPI-01, D-01/D-02)",
    async () => {
...
describe("classifyRows", () => {
  it.fails(
    "assigns covered, deferred, out-of-scope, and gap buckets per D-04",
    async () => {
...
describe("assertCoverageFresh", () => {
  it.fails(
    "throws or exits non-zero when docs/COVERAGE.md is missing or stale (D-06)",
    async () => {
      const { assertCoverageFresh } = await import(
        "../scripts/openapi-coverage.mjs"
      );

      expect(() => assertCoverageFresh()).toThrow(/COVERAGE|stale|missing/i);
    },
  );
});
