...
4. On watch timeout error: re-call \`deployment.watch\` with the same \`deployment_uuid\` (raise \`timeout\` if builds are slow). On \`failed\` or \`cancelled\`: surface the error and logs hint to the user — do not treat as success.
