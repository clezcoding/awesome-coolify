...
1. **Extract `syncApplicationEnv` helper?** — RESOLVED
   ...
   - **Resolution:** Inline delegation in Plan 23.1-01 (no helper).

2. **Wave ordering: set_env vs Nyquist first?** — RESOLVED
   ...
   - **Resolution:** Wave order 01 → 02/03 (Plan 01 set_env first; Plans 02–03 Nyquist after).

3. **Phase 19 frontmatter `nyquist_compliant: true` while `status: draft`?** — RESOLVED
   ...
   - **Resolution:** validate-phase resets Phase 19 frontmatter from earned state only (Plan 23.1-02).
